...
class CleanDataPipeline:
    ...
    def load_clean_save_data(
        ...
        if self.file_type == "zip":
            data_processor = DataProcessorZip("data/eurostat_life_expect.zip")            
    ...
    def check_if_file_type_is_valid(self) -> None:
        if self.file_type not in ["csv", "zip"]:
            raise ValueError("Invalid data format. Supported formats are 'csv' and 'zip'.")
    ...
    def save_file_to_csv(self,df_melt: pd.DataFrame) -> None:
        ...
        """
        df_melt.to_csv(f"data/{str(self.region).lower()}_life_expectancy.csv", sep=",", index=False)


if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(description="Example argparse")
    parser.add_argument("--region", "-r", type=Country, choices= Country, help="Region name")
    parser.add_argument(
        "--format",
        "-f",
        type=str,
